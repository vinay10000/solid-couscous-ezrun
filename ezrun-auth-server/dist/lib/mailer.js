...
async function sendViaSmtp(args) {
    ...
    try {
        ...
        const shouldRetryWithFallback = code === "ETIMEDOUT" ||
            code === "ECONNECTION" ||
...
export async function sendOtpEmail(args) {
    ...
    throw new Error(`No email provider could send OTP. ${errors.join(" | ") || "No provider configured."}`);
}
